...
def mcp_flow_new(
    ...
    *,
    ...
    # See: packages/pipeio/docs/specs/pipeio/pipeline-docs.md
    ...
    if not docs_index.exists():
        ...
        if not legacy_overview.exists():
            ...
            docs_index.write_text(
                ...
                f"## Purpose\n"
                f"\n"
                f"<!-- One paragraph: what this flow produces and why it exists.\n"
                f"     What downstream flows consume its output? -->\n"
                f"\n"
                f"## Input\n"
                f"\n"
                f"- Input directory: `{_input_dir}`\n"
                f"- Input manifest: `{_input_manifest}`\n"
                f"- Modality: <!-- data format/source -->\n"
                f"\n"
                f"## Output\n"
                f"\n"
                ...
                f"\n"
                f"### Final products (per session)\n"
                f"\n"
                f"<!-- Table of the actual files this flow writes per session.\n"
                f"     A supervisor should be able to answer 'what does this flow\n"
                f"     produce?' from this table alone. -->\n"
                f"\n"
                f"| File | Content |\n"
                f"|------|---------|\n"
                f"| | |\n"
                ...
                f"## Mod Chain\n"
                f"\n"
                f"<!-- Processing order with rationale. The Rules column lists\n"
                f"     the actual snakemake rule names for each mod. -->\n"
                f"\n"
                f"| Order | Mod | Rules | Purpose |\n"
                f"|-------|-----|-------|---------|\n"
                f"| 1 | | | |\n"
                f"\n"
                f"## Design Decisions\n"
                ...
                f"     raw vs intermediate? Each bullet should have a rationale,\n"
                f"     not just a statement. -->\n"
                f"\n"
                f"## Data Availability\n"
                f"\n"
                f"<!-- Session/subject coverage summary. E.g.:\n"
                f"     '48 sessions across 5 subjects. Sub-05 has best coverage\n"
                f"     (25 sessions). Sub-01 only has 4 homecage sessions.' -->\n"
                f"\n"
                f"## DAG\n"
                f"\n"
                f"![Rule DAG](dag.svg)\n"
                f"\n"
                f"<!-- Generated by `pipeio flow dag {flow}`. -->\n"
                f"\n"
                f"## Report\n"
                f"\n"
                f"See [Snakemake report](report.md) — interactive HTML with QC\n"
                f"plots and runtime stats.\n"
                f"\n"
                f"## Changelog\n"
                f"\n"
                f"See [CHANGELOG](changelog.md) for design history: strategy shifts,\n"
                f"DAG reorganizations, and key discoveries driving design.\n"
                f"\n"
                f"## Known Gaps\n"
                f"\n"
    ...
    # CHANGELOG.md — flow-root design history
    # See: packages/pipeio/docs/specs/pipeio/pipeline-docs.md (CHANGELOG section)
    changelog_path = flow_dir / "CHANGELOG.md"
    if not changelog_path.exists():
        changelog_path.write_text(
            f"# Changelog — {flow}\n"
            f"\n"
            f"High-level changes to the {flow} pipeline: strategy shifts, DAG\n"
            f"reorganizations, and key discoveries that drive design decisions.\n"
            f"Minor fixes and parameter tweaks belong in git log, not here.\n"
            f"\n"
            f"## Unreleased\n"
            f"\n"
            f"<!-- Add dated entries above this line in reverse chronological\n"
            f"     order. Format:\n"
            f"\n"
            f"     ## YYYY-MM-DD — Short one-line title\n"
            f"\n"
            f"     **Strategy change:** one-paragraph summary\n"
            f"\n"
            f"     **Why:**\n"
            f"     - bullet point\n"
            f"\n"
            f"     **Refs:**\n"
            f"     - [meeting note](../../../docs/log/meeting/...)\n"
            f"-->\n",
            encoding="utf-8",
        )
        created.append("CHANGELOG.md")

    # publish.yml — collection gating for docs_collect
    publish_path = flow_dir / "publish.yml"
    if not publish_path.exists():
        publish_path.write_text(
            "# Controls which optional artifacts docs_collect publishes.\n"
            "# See: packages/pipeio/docs/specs/pipeio/pipeline-docs.md\n"
            "dag: true            # DAG SVG\n"
            "report: true         # snakemake --report HTML\n"
            "scripts: true        # scripts index page\n"
            "changelog: true      # CHANGELOG.md → changelog.md\n",
            encoding="utf-8",
        )
        created.append("publish.yml")
...
def mcp_flow_report(
    ...
    Pre-flight size check: snakemake ``--report`` base64-embeds every
    target file into the HTML, so dense SVG plots can balloon to GB-scale
    reports. Before invoking snakemake, the total byte size of resolved
    targets is summed. If it exceeds ``max_embed_mb``, the tool refuses
    with the top offenders listed; if it exceeds ``warn_embed_mb``, it
    proceeds but returns a warning. Pass ``force=True`` to bypass the
    hard limit.
    ...
    Args:
        ...
        max_embed_mb: Hard limit on summed target size (MB). Defaults to 200.
        warn_embed_mb: Soft threshold for warnings (MB). Defaults to 50.
        force: Bypass the hard limit when True.
    """
    import subprocess
